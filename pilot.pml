...
ltl never_reaches_adscom {
	[]!(received_data[0].device == adscom ||
			received_data[1].device == adscom ||
			received_data[2].device == adscom)
};
...
// Can Adscom use Alice's data for other purpose than commercial offers?
ltl transfer_to_adscom_according_to_policy {
		[](received_data[1].device == adscom ->
...
ltl can_carinsure_use_for_profiling {
	[](received_data[2].device == carinsure ->
		 received_data[2].policy.dcr.dur.purpose != profiling)
};
...
mtype = {
	...
	alice, parket, adscom, carinsure,     /* Devices \Delta & Entities \mahtcal{E} */
	commertial_offers, profiling,         /* Purposes \mathcal{P}                  */
...
inline policy_generator(p_entity, result_policy) {
	...
	result_policy.dcr.entity = p_entity; /*Collecting (requesting) entity*/
	result_policy.dcr.dur.purpose = commertial_offers;
	result_policy.dcr.dur.retention_time = 42;

	/*TRANSFER RULES*/
	/*TRANSFER RULE 1*/
	result_policy.tr[0].condition = true;
	result_policy.tr[0].entity = parket;
	result_policy.tr[0].dur.purpose = commertial_offers;
	result_policy.tr[0].dur.retention_time = 42;
	/*TRANSFER RULE 2*/
	result_policy.tr[1].condition = true;
	result_policy.tr[1].entity = adscom;
	result_policy.tr[1].dur.purpose = commertial_offers;
...
inline active_transfer_rule(p_pol,
							...
							result) {
	if
		:: (/* p_pol.dur.retention_time >= clock             &&  */ // Uncomment only for verification 
			  /* p_transfer_rule.dur.retention_time >= clock   &&  */ // of retention time properties.
...
proctype deviceAlice() {
	byte stop=false;
	do
		:: stop -> break; // Stop after data send (only for verification purposes)
	  // Send alice -> parket
		:: atomic{(policy_base[0].device == alice && policy_base[0].owner == parket  ||
							 policy_base[1].device == alice && policy_base[1].owner == parket  ||
							 policy_base[1].device == alice && policy_base[2].owner == parket  ||
							 policy_base[3].device == alice && policy_base[3].owner == parket  ||
							 policy_base[4].device == alice && policy_base[4].owner == parket) ->
							printf("{send(%e,%e,%e)}",alice,parket,database[0].item.item_id);
							send(alice, parket, database[0].item);
							stop=true;}
	od;
};
...
proctype deviceParket() {
	...
	do
	  ...
	  // Request parket -> alice
		...
		// Transfer parket -> adscom
		:: atomic{(policy_base[0].device == parket && policy_base[0].owner == adscom ||
							 policy_base[1].device == parket && policy_base[1].owner == adscom ||
							 policy_base[2].device == parket && policy_base[2].owner == adscom ||
							 policy_base[3].device == parket && policy_base[3].owner == adscom ||
							 policy_base[4].device == parket && policy_base[4].owner == adscom) &&
							(database[1].device == parket && database[1].item.item_id != 0) ->
							printf("{transfer(%e, %e, %e)}\n",parket, adscom, database[1].item.item_id);
							transfer(parket,adscom,database[1].item);}
		:: stop = true;
	od;
};

proctype deviceAdscom() {
	Policy rand_pol;
	do
		:: atomic{
			policy_generator(adscom,rand_pol);
			printf("{request(%e, %e, %e, (%e,<%d,%e,<%e,%d>>,", adscom,
																													parket,
																													rand_pol.datatype,
																				 									rand_pol.datatype,
																				 									rand_pol.dcr.condition,
			...
			printf("))}\n");
			request(adscom,parket,rand_pol.datatype,rand_pol);
			/* stop=true; */
		}
		:: atomic{bool r_result = false;
							illegal_use(adscom,plate_alice_1,advertisement,r_result);
							printf("{illegal_use(%e, %e, %e); succeed = %d}\n",adscom,plate_alice_1,advertisement,r_result);
						 }
		// Illegal Transfer adscom -> carinsure (second position database)
	  :: atomic{(database[2].device == adscom && database[2].item.item_id != 0) ->
	 						printf("{illegal_transfer(%e, %e, %e)}\n", adscom, carinsure, database[2].item.item_id);
	 						illegal_transfer(adscom,carinsure,database[2].item);}
	od;
};
...
/* /\* Devices *\/ */
/* mtype:device      =  {alice,adscom,parket}; */
/* /\* Obligations *\/ */
